...
export class AISystem {
  ...
  isBusinessRelated(text, business) {
    ...
    // Always allow greetings and basic conversational starters
    const greetings = ['hello', 'hi', 'hey', 'good morning', 'good afternoon', 'good evening', 'how are you', 'thanks', 'thank you', 'bye', 'goodbye'];
    const isGreeting = greetings.some(greeting => t.includes(greeting));
    
    if (isGreeting) {
      return true; // Always allow greetings
    }
    ...
    // General knowledge questions (NOT business related) - but exclude greetings
  ...
  generateFallbackResponse(business, query) {
    ...
    switch (intent) {
      case 'greeting':
        return `Hello! Welcome to ${business.name}. How can I help you today? I can provide information about our services, pricing, location, contact details, or hours.`;
      
      case 'thanks':
        return `You're welcome! Is there anything else about ${business.name} I can help you with?`;
      
      case 'goodbye':
        return `Thank you for contacting ${business.name}! Have a great day, and feel free to reach out anytime if you need assistance.`;
      ...
      case 'menu':
      case 'inquiry':
        return `Thank you for your interest in ${business.name}! I'd love to provide you with detailed information. Please reach out to us directly so we can discuss your specific needs:\n\n${this.buildFallbackContactCard(business)}`;
      
      case 'booking':
      case 'reservation':
        return `I'd be happy to help you with booking! Please contact us using the details below and we'll assist you:\n\n${this.buildFallbackContactCard(business)}`;
...
RESPONSE GUIDELINES:
- Always respond warmly to greetings and basic conversational starters.
...
- Stay focused on ${business.name} related topics, but be conversational and friendly.
- Welcome users with greetings like "Hello! How can I help you with ${business.name} today?"`;
...
Remember: Welcome greetings warmly and respond to queries specifically related to ${business.name} - services, products, pricing, contact info, location, hours, booking, etc. Do not answer general knowledge questions, but always be friendly with basic conversation.`;
  }
  ...
  analyzeIntent(message) {
    ...
    const intents = {
      ...
      pricing: ['price', 'cost', 'fee', 'charge', 'money', 'expensive', 'cheap', 'budget', 'rate', 'rates'],
      services: ['service', 'services', 'offer', 'provide', 'available', 'what do you'],
      products: ['product', 'products', 'item', 'items', 'menu', 'food', 'drink'],
      schedule: ['time', 'schedule', 'timing', 'when', 'hours', 'open', 'close', 'available'],
      booking: ['book', 'booking', 'reserve', 'reservation', 'appointment', 'order'],
      contact: ['contact', 'call', 'phone', 'email', 'address', 'location', 'visit', 'whatsapp', 'connect me', 'speak to someone', 'human', 'agent'],
      delivery: ['delivery', 'deliver', 'pickup', 'takeaway', 'home delivery'],
      policies: ['policy', 'refund', 'terms', 'conditions', 'rules', 'cancellation'],
  ...
  shouldShowContactForm(intent, message) {
    const contactTriggers = [
      ...
      'connect me', 'talk to', 'reach out', 'get in touch'
    ];
