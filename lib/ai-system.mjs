...
export class AISystem {
  ...
  isBusinessRelated(text, business) {
    ...
    const businessKeywords = [
      ...
      businessName, 'jspiders', 'academy', 'institute', 'training', 'course', 'program',
      'class', 'batch', 'syllabus', 'curriculum', 'fee', 'price', 'cost', 'admission',
      'enroll', 'register', 'join', 'apply', 'eligibility', 'placement', 'job',
      'career', 'certification', 'duration', 'schedule', 'timing', 'demo', 'trial',
      
      // Contact and location related to business
      'contact', 'phone', 'call', 'whatsapp', 'email', 'address', 'location',
      'visit', 'hours', 'timings', 'directions', 'how to reach', 'where',
      
      // Business services
      'coding', 'programming', 'java', 'python', 'web development', 'software',
      'fullstack', 'backend', 'frontend', 'database', 'technology', 'tech',
      
      // Business policies
      'refund', 'policy', 'terms', 'conditions', 'rules', 'cancellation',
      
      // Travel related to business location (acceptable)
      'far from', 'distance', 'travel time', 'how long', 'transport', 'bus', 'metro',
      'taxi', 'auto', 'train', 'flight'
    ];
    ...
    const isTravelToLocation = (
      ...
      t.includes('travel') || t.includes('reach')
    ) && (
    ...
    const generalKnowledgePatterns = [
      ...
      'sports', 'game', 'movie', 'book', 'song', 'music', 'food', 'recipe'
    ];
    ...
    if (isGeneralKnowledge && !hasBusinessKeywords && !isTravelToLocation) {
      return false;
    }
  ...
  generateDeclineMessage(business) {
    const responses = [
      ...
      `I can only assist with queries related to ${business.name} - our courses, admissions, contact details, and services. For other questions, please try a general search engine.`,
    ...
    return `${decline}\n\nIs there anything specific about ${business.name} I can help you with? I can provide information about our courses, fees, schedules, admissions, or contact details.`;
  }
